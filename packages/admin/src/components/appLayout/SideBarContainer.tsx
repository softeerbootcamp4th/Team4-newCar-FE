...
function SideBarContainer() {
  ...
  return (
	<nav role="navigation" className="mt-5">
		<div id="menuToggle" className="relative z-10 select-none">
			<input
  type="checkbox"
  checked={isChecked}
  onChange={handleToggle}
  className="w-10 h-8 absolute top-[-7px] left-[-5px] cursor-pointer opacity-0 z-20"
			/>

			<span
  className={`${spanStyled} ${
        isChecked ? 'transform rotate-45 translate-x-[-2px] translate-y-[-1px] bg-[#232323]' : ''
      }`}
			/>
			<span
  className={` ${spanStyled} ${
        isChecked ? 'opacity-0 transform rotate-0 scale-[0.2,0.2]' : ''
			...
			<span
  className={`${spanStyled}  ${
        isChecked ? 'transform rotate-[-45deg] translate-x-[-2px translate-y-[-1px] bg-[#232323]' : ''
      }`}
			/>
